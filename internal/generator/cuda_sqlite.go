...
// CUDASQLiteGenerator generates CUDA code with SQLite-backed deduplication for unlimited trace generation.
type CUDASQLiteGenerator struct {
	deviceCode strings.Builder
	hostCode   strings.Builder
}

// NewCUDASQLiteGenerator creates a new CUDA generator with SQLite integration.
func NewCUDASQLiteGenerator() *CUDASQLiteGenerator {
	return &CUDASQLiteGenerator{}
}

// GenerateCUDAWithSQLite generates CUDA source code that includes SQLite for trace deduplication.
// This allows unlimited traces without memory accumulation, matching tgeval's approach but with GPU acceleration.
func (g *CUDASQLiteGenerator) GenerateCUDAWithSQLite(schema *model.Schema, scope int) (string, error) {
	g.generateIncludes()
...
// generateIncludes writes CUDA header includes plus SQLite for deduplication.
func (g *CUDASQLiteGenerator) generateIncludes() {
...
func (g *CUDASQLiteGenerator) generateDeviceTypes(schema *model.Schema) {
...
func (g *CUDASQLiteGenerator) generateKernels(schema *model.Schema, _ int) {
...
// generateHostWrapper writes the host-side wrapper with SQLite deduplication support.
func (g *CUDASQLiteGenerator) generateHostWrapper(schema *model.Schema, scope int) {
	numRoots := 0
	for _, rule := range schema.Rules {
		if rule.IsRoot {
			numRoots++
	...
	h.WriteString(`
// ============================================================
// Host-side driver: memory management, kernel launch, result extraction with SQLite deduplication
// ============================================================
...
`)
	...
	// Multi-GPU support with SQLite deduplication
	h.WriteString(`
    ...
    if (num_devices == 0) {
        ...
        // Run single-threaded fallback with SQLite deduplication
        ...
        if (init_trace_store(&trace_store, db_path) == 0) {
            // Generate simple fallback trace
            DeviceTraceSegment dummy;
            memset(&dummy, 0, sizeof(dummy));
            dummy.mark_status = 'U';
            dummy.total_prob = 1.0;

            char events_json[4096];
            snprintf(events_json, sizeof(events_json), "[[\"fallback\", \"R\", 1, 0]]");

            unsigned int hash = 2166136261;
            for (int ci = 0; dummy.events[0].name[ci] != '\0' && ci < 63; ci++) {
                hash ^= (unsigned char)dummy.events[0].name[ci];
                hash *= 16777619;
            }
            char trace_key[64];
            snprintf(trace_key, sizeof(trace_key), "%08x", hash);

            insert_trace_to_store(&trace_store, trace_key, "U", dummy.total_prob, events_json);
            close_trace_store(&trace_store);
        }
        return 0;
    }

    printf("CUDA: Found %d device(s). Using GPU-accelerated trace generation with SQLite deduplication.\n", num_devices);
...
`)
    ...
    // Copy results back and insert into SQLite for deduplication (matches tgeval approach)
`)

	h.WriteString(`
    int total_traces = 0;
    for (int d = num_devices - 1; d >= 0 && total_traces < num_segments + start_segment; d--) {
        ...
        for (int i = 0; i < device_seg_counts[d] && total_traces < num_segments; i++) {
            ...
            // Serialize events to JSON string for SQLite storage
            char events_json[4096];
            int pos = 0;
            pos += snprintf(events_json + pos, sizeof(events_json) - pos, "[");
            for (int j = 0; j < seg->count && j < MAX_EVENTS_PER_SEGMENT; j++) {
                if (j > 0) pos += snprintf(events_json + pos, sizeof(events_json) - pos, ", ");

                // Format: [name, type, position, root_index]
                char event_name[64];
                int name_len = 0;
                for (int ci = 0; seg->events[j].name[ci] != '\0' && ci < 63; ci++) {
                    event_name[name_len++] = seg->events[j].name[ci];
                }
                ...
                pos += snprintf(events_json + pos, sizeof(events_json) - pos,
                               "[\"%s\", \"%c\", %d, %d]",
                               event_name,
                               seg->events[j].type == EVENT_TYPE_ROOT ? 'R' : 'A',
                               seg->events[j].position,
                               seg->events[j].root_index);
            }
            pos += snprintf(events_json + pos, sizeof(events_json) - pos, "]");

            // Generate trace key based on event sequence (FNV-1a hash, matches Go version)
            char trace_key[64];
            unsigned int hash = 2166136261;
            for (int j = 0; j < seg->count && j < MAX_EVENTS_PER_SEGMENT; j++) {
                for (int ci = 0; seg->events[j].name[ci] != '\0' && ci < 63; ci++) {
                    hash ^= (unsigned char)seg->events[j].name[ci];
                    hash *= 16777619; // FNV prime
                }
            }
            snprintf(trace_key, sizeof(trace_key), "%08x", hash);

            // Insert into SQLite store for deduplication (matches tgeval behavior)
            if (insert_trace_to_store(&trace_store, trace_key, mark, seg->total_prob, events_json) != 0) {
                fprintf(stderr, "Failed to insert trace %d\n", total_traces);
            } else {
                total_traces++;
            }
        }
    ...
    // Output pretty-printed JSON from SQLite (matches tgeval format exactly)
`)

	h.WriteString(`
    printf("\n[INFO] Generated %d unique traces (%d total instances)\n", total_traces, total_traces);
    printf("{\n");
    printf("  \"traces\":[\n");

    sqlite3_stmt* select_stmt;
    const char* select_sql = "SELECT trace_key, mark_status, probability, events_json, follows_pairs_json, in_pairs_json, udrs_json, views_json, count FROM traces ORDER BY trace_key";

    if (sqlite3_prepare_v2(trace_store.db, select_sql, -1, &select_stmt, NULL) == SQLITE_OK) {
        int trace_count = 0;
        while (sqlite3_step(select_stmt) == SQLITE_ROW) {
            const char* key = (const char*)sqlite3_column_text(select_stmt, 0);
            const char* mark = (const char*)sqlite3_column_text(select_stmt, 1);
            double prob = sqlite3_column_double(select_stmt, 2);
            const char* events = (const char*)sqlite3_column_text(select_stmt, 3);
            int count = sqlite3_column_int(select_stmt, 8);

            if (trace_count > 0) printf(",\n");

            // Format: [mark_status, probability, events_json, follows_pairs, in_pairs, views, count]
            printf("    [%s, %f, %s, [], [], {}, %d]", mark, prob, events, count);
            trace_count++;
        }
        printf("\n");
        ...
        fprintf(stderr, "SQLite select failed: %s\n", sqlite3_errmsg(trace_store.db));
    }
...
func (g *CUDASQLiteGenerator) GenerateCUDADryRun(code string) error {
